...
def make_info_field(columnNames, columns):
    ...
    """
    infoField = [
        "=".join((cn, vcfhelper.translate_illegal(c)))
        for cn, c in zip(columnNames, columns)
        if not cn in GT_COLS and not c.strip() == ""
    ]
...
def make_genotype_field(columnNames, columns):
    ...
    return ":".join(
        (
            vcfhelper.translate_illegal(c) or "."
            for cn, c in zip(columnNames, columns)
            if cn in GT_COLS
        )
    )
...
def make_vcf(csvReader, vcfAlleleCreator, pcs):
    ...
    assert (
        ...
        and columnNames[1].lower() == "hgvs"
        and columnNames[2].lower() == "homozygous"
    )
    ...
    for cn in columnNames[3:]:
        if cn in GT_COLS:
            sys.stdout.write(
                '##FORMAT=<ID={0},Number=.,Type=String,Description="{0}">\n'.format(cn)
            )
    ...
    for columns in csvReader:
        ...
        try:
            ...
            sys.stderr.write(
                "Error: Skipping {} as it does not conform to HGVS standard!\n".format(hgvsField)
            )
            continue
        ...
        if re.search(r">", hgvsVar):
            chromosome, vcfPosition, vcfID, ref, alt = from_SNP(
                vcfAlleleCreator, hgvsVar, chromosome, hgvsStart, sampleID
            )
        elif re.search(r"delins|del\S+ins", hgvsVar):
            chromosome, vcfPosition, vcfID, ref, alt = from_indel(
                vcfAlleleCreator, hgvsVar, chromosome, hgvsStart, hgvsEnd, sampleID
            )
        elif re.search(r"ins", hgvsVar):
            chromosome, vcfPosition, vcfID, ref, alt = from_insertion(
                vcfAlleleCreator, hgvsVar, chromosome, hgvsStart, sampleID
            )
        elif re.search(r"del", hgvsVar):
            chromosome, vcfPosition, vcfID, ref, alt = from_deletion(
                vcfAlleleCreator, hgvsVar, chromosome, hgvsStart, hgvsEnd, sampleID
            )
        elif re.search(r"dup", hgvsVar):
            chromosome, vcfPosition, vcfID, ref, alt = from_duplication(
                vcfAlleleCreator, hgvsVar, chromosome, hgvsStart, hgvsEnd, sampleID
            )
        else:
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="""Convert CSV file with HGVS-described sample/variant data to a VCF file.
    ...
    vcfAlleleCreator = vcfhelper.VCFAlleleCreator(
        Fasta(filepath=args.fasta), useGenomeRef=not args.hgvsRef
    )
    pcs = posconverter.PosConverterService.make_pos_converters(
        args.refgeneFilePath
    )  # Will give warnings
