...
class DepositAlleles(object):
    ...
    def get_genepanel(self, allele_config):
        genepanel_name = allele_config["params"]["genepanel"].split('_')[0]
        genepanel_version = allele_config["params"]["genepanel"].split('_')[1]
        try:
...
def main(argv=None):
    ...
    parser.add_argument("--analysiscfg", action="store", dest="allele_config_file", required=True,
    ...
    with open(args.allele_config_file) as f:
        allele_config = json.load(f)
