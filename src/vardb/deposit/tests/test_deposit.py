...
def test_analysis_single(test_database, session):
    ...
    real_variants = [
        v(
            '10', '89623201', 'A', 'T',
            [s('0/1', '0,101', '301', '91', '0,301,400')]
        ),
        v(
            '10', '89623202', 'A', 'C',
            [s('0/1', '0,102', '302', '92', '0,302,400')]
        ),
        v(
            '10', '89623203', 'G', 'T',
            [s('1/1', '0,103', '303', '93', '0,303,400')]
        ),
        # Biallelic site
        v(
            '10', '89623204', 'A', 'T',
            [s('1/.', '0,104', '304', '94', '0,304,400')]
        ),
        v(
            '10', '89623205', 'A', 'T',
            [s('./1', '0,105', '305', '95', '0,305,400')]
        ),
        # Biallelic in opposite order
        v(
            '10', '89623206', 'A', 'T',
            [s('./1', '0,106', '306', '96', '0,306,400')]
        ),
        v(
            '10', '89623207', 'A', 'T',
            [s('1/.', '0,107', '307', '97', '0,307,400')]
        ),
    ]

    non_variants = [
        # Proband 0/0 -> not imported
        v(
            '10', '89623208', 'A', 'T',
            [s('0/0', '0,108', '808', '98', '0,308,400')]
        ),
        # Proband ./. -> not imported
        v(
            '10', '89623209', 'A', 'T',
            [s('./.', '0,109', '309', '99', '0,309,400')]
        ),
    ]

    variants = real_variants + non_variants

    vcf = create_vcf(variants, samples)

    da = DepositAnalysis(session)
    acd = AnalysisConfigData(vcf, analysis_name, 'HBOCUTV', 'v01', 1)
    da.import_vcf(acd)

    # Check analysis + samples
    analysis = session.query(sample.Analysis).filter(
        sample.Analysis.name == analysis_name
    ).one()

    assert len(analysis.samples) == 1
    assert analysis.samples[0].identifier == samples[0]
    assert analysis.samples[0].proband is True
    assert analysis.samples[0].affected is True
    sample_id = analysis.samples[0].id

    # Check genotypes + genotypesampledata
    genotypes = session.query(genotype.Genotype).filter(
        genotype.Genotype.sample_id.in_([sa.id for sa in analysis.samples])
    ).all()

    # 3 normal + 4 biallelic -> 5 genotypes
    assert len(genotypes) == 5

    for variant in real_variants:
        gts = [gt for gt in genotypes if (gt.allele.start_position == int(variant['pos']) - 1) or (gt.secondallele and gt.secondallele.start_position == int(variant['pos']) - 1)]
        assert len(gts) == 1
        gt = gts[0]
        assert gt.variant_quality == 5000
        assert gt.filter_status == 'PASS'
        sample_data = variant['samples'][0]

        # Not biallelic
        if sample_data['GT'] in ['0/1', '1/1']:
            assert len(gt.genotypesampledata) == 1
            gsd = gt.genotypesampledata[0]
            assert gsd.sample_id == sample_id
            if sample_data['GT'] == '1/1':
                assert gsd.type == 'Homozygous'
            elif sample_data['GT'] == '0/1':
                assert gsd.type == 'Heterozygous'
            assert gsd.multiallelic is False
            assert gsd.secondallele is False
            assert gsd.genotype_quality == int(sample_data['GQ'])
            assert gsd.sequencing_depth == int(sample_data['DP'])
            assert gsd.genotype_likelihood == [int(p) for p in sample_data['PL'].split(',')]
            ads = [int(a) for a in sample_data['AD'].split(',')]
            assert gsd.allele_depth == {k: v for k, v in zip(['REF', variant['alt']], ads)}

        # Biallelic
        elif sample_data['GT'] in ['./1', '1/.']:
            assert len(gt.genotypesampledata) == 2
            gsd1, gsd2 = gt.genotypesampledata
            assert gsd1.sample_id == sample_id
            assert gsd2.sample_id == sample_id
            assert gsd1.multiallelic is True
            assert gsd2.multiallelic is True
            assert gsd1.secondallele is False and gsd2.secondallele is True or \
                   gsd1.secondallele is True and gsd2.secondallele is False
            assert gsd1.type == 'Heterozygous'
            assert gsd2.type == 'Heterozygous'
            # For decomposed vcf, PL is wrong for multiallelic sites.
            # Make sure we don't import it
            assert gsd1.genotype_likelihood is None
            assert gsd2.genotype_likelihood is None
            # Since both sites have same REF, we don't import allele depth
            assert gsd1.allele_depth == {}
            assert gsd2.allele_depth == {}
            assert gsd1.genotype_quality == int(sample_data['GQ']) or \
                   gsd2.genotype_quality == int(sample_data['GQ'])
            assert gsd1.sequencing_depth == int(sample_data['DP']) or \
                   gsd2.sequencing_depth == int(sample_data['DP'])
