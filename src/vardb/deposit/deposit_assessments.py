    ...
    def get_genepanel(self, genepanel_name, genepanel_version):
        if genepanel_name is None or genepanel_version is None:
            ...
            try:
                ...
                log.warning(
                    "Genepanel {} version {} not available in varDB".format(
...
def main(argv=None):
    ...
    parser.add_argument(
        "--vcf", action="store", dest="vcfPath", required=True, help="Path to VCF file to deposit"
    )
    parser.add_argument(
        "--genepanel-name",
        action="store",
        dest="gp_name",
        required=True,
        help="Name of genepanel for assessments",
    )
    parser.add_argument(
        "--genepanel-version",
        action="store",
        dest="gp_version",
        required=True,
        help="Version of genepanel for assessments",
    )
    parser.add_argument(
        "--update-annotation",
        action="store_true",
        dest="update_annotations",
        help="Flag to update annotations",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Do not ask for confirmation before deposit",
    )
