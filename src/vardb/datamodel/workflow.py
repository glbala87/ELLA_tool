...
class InterpretationSnapshotMixin(object):
    ...
    id = Column(Integer, primary_key=True)
    date_created = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.datetime.now(pytz.utc))
    filtered = Column(Enum("FREQUENCY", "REGION", "POLYPYRIMIDINE", "GENE", "QUALITY", "SEGREGATION",  name="interpretationsnapshot_filtered"),)  # If the allele was filtered, this describes which type of filtering
