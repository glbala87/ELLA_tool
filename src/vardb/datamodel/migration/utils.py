...
def update_enum(op, table_names, column, enum_name, old_options, new_options, nullable=False):
    ...
    for table_name in table_names:
        ...
        op.execute(
            ALTER_TABLE_TEMPLATE.format(
                table_name=table_name, column=column, enum_type=tmp_type_name
            )
        )
    ...
    for table_name in table_names:
        op.execute(
            ALTER_TABLE_TEMPLATE.format(table_name=table_name, column=column, enum_type=enum_name)
        )
