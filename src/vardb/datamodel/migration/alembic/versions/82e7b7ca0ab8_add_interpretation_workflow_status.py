...
def migrate_interpretations(conn, model, id_attr):
    ids = conn.execute(sa.select([getattr(model.c, id_attr)]).distinct())
    for obj_id in ids:
        obj_id = obj_id[0]
        interpretations = conn.execute(
            sa.select([model.c.id, model.c.end_action])
            .where(getattr(model.c, id_attr) == obj_id)
            .order_by(model.c.date_last_update)
        )
        ...
        for idx, i in enumerate(interpretations):
            ...
            conn.execute(sa.update(model).where(model.c.id == i.id).values(**values))
...
def upgrade():
    ...
    workflow_status = sa.Enum(
        "Interpretation", "Review", name="alleleinterpretation_workflow_status"
    )
    ...
    op.add_column(
        "alleleinterpretation", sa.Column("workflow_status", workflow_status, nullable=True)
    )
    ...
    op.add_column(
        "analysisinterpretation", sa.Column("workflow_status", workflow_status, nullable=True)
    )
