...
def upgrade():
    ...
    op.drop_constraint("uq_gene_hgnc_symbol", "gene", type_="unique")
    op.execute(
        "CREATE UNIQUE INDEX ix_gene_hgnc_symbol ON gene USING btree(lower(hgnc_symbol) text_pattern_ops)"
    )
    ...
    op.execute(
        "CREATE INDEX ix_annotationshadowtranscript_hgvsc ON annotationshadowtranscript USING btree(lower(hgvsc) text_pattern_ops)"
    )
