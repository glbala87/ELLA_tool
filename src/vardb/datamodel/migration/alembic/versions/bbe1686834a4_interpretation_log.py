...
def upgrade():
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["alleleinterpretation.id"],
            name=op.f("fk_interpretationlog_alleleinterpretation_id_alleleinterpretation"),
        ),
        sa.ForeignKeyConstraint(
            ["analysisinterpretation_id"],
            ["analysisinterpretation.id"],
            name=op.f("fk_interpretationlog_analysisinterpretation_id_analysisinterpretation"),
        ),
        sa.ForeignKeyConstraint(
            ["user_id"], ["user.id"], name=op.f("fk_interpretationlog_user_id_user")
        ),
    ...
    for ai_id, user_id, review_comment, date_last_update in al_review_comments:
        conn.execute(
            interpretation_log_table.insert().values(
