...
def upgrade():
    ...
    for phenotypes in list(ordered_phenotypes.values()):
        ...
        for genepanel in genepanels:
            phenotypes = conn.execute(
                "INSERT INTO genepanel_phenotype (phenotype_id, genepanel_name, genepanel_version) VALUES ({}, '{}', '{}')".format(
                    p.id, genepanel[0], genepanel[1]
                )
            )

    op.create_foreign_key(
        op.f("fk_genepanel_user_id_user"), "genepanel", "user", ["user_id"], ["id"]
    )
