...
analysisinterpretationsnapshot = sa.sql.table('analysisinterpretationsnapshot',
                   sa.Column('interpretationsnapshot_filtered', new_type, nullable=True))
alleleinterpretationsnapshot = sa.sql.table('analysisinterpretationsnapshot',
                   sa.Column('interpretationsnapshot_filtered', new_type, nullable=True))
...
def upgrade():
    ...
    op.execute('ALTER TABLE analysisinterpretationsnapshot ALTER COLUMN filtered TYPE tmp_interpretation_snapshot_filtered'
               ' USING filtered::text::tmp_interpretation_snapshot_filtered')
    op.execute('ALTER TABLE alleleinterpretationsnapshot ALTER COLUMN filtered TYPE tmp_interpretation_snapshot_filtered'
               ' USING filtered::text::tmp_interpretation_snapshot_filtered')
    old_type.drop(op.get_bind(), checkfirst=False)

    # Update values to match new ENUM
    op.execute("UPDATE analysisinterpretationsnapshot SET filtered='REGION' where filtered='UTR' or filtered='INTRON'")

    # Create and convert to the "new" interpretationsnapshot_filtered type
    new_type.create(op.get_bind(), checkfirst=False)
    op.execute('ALTER TABLE analysisinterpretationsnapshot ALTER COLUMN filtered TYPE interpretationsnapshot_filtered'
               ' USING filtered::text::interpretationsnapshot_filtered')
    op.execute('ALTER TABLE alleleinterpretationsnapshot ALTER COLUMN filtered TYPE interpretationsnapshot_filtered'
               ' USING filtered::text::interpretationsnapshot_filtered')
