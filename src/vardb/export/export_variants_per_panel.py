...
def export_variants_per_panel(session, output):
    ...
    for gp_key in gp_keys:
        ...
        allele_ids = session.query(allele.Allele.id).join(
    ...
    filtered_results = AlleleFilter(session).filter_alleles(gp_allele_ids)
    for gp_key, data in filtered_results.iteritems():
        ...
        allele_classification = session.query(
        ...
        for c in CLASSIFICATIONS:
            row.append(
                WriteOnlyCell(worksheet, allele_classification_cnt.get(c, 0))
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", required=True, action="store", dest="output", help="Name of output file")
