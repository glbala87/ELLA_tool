...
def get_variant_rows(session, filter_config_id, ids_not_started):
    ...
    analyses_allele_ids = session.query(
...
def get_warning_rows(session, ids_not_started):
    ...
    analyses_with_warnings = session.query(
        sample.Analysis
    ).filter(
        sample.Analysis.id.in_(ids_not_started),
    ...
    for analysis in analyses_with_warnings:
        ...
        for match in re.finditer(WARNING_REGIONS_RE, analysis.warnings):
            ...
            warning_rows.append(
                create_warning_row(analysis_info, warning_info)
            )
...
def export_variants(session, genepanels, filter_config_id, excel_file_obj, csv_file_obj=None):
    ...
    ids_not_started = session.query(workflow.AnalysisInterpretation.analysis_id).join(
        sample.Analysis
    ).filter(
        tuple_(sample.Analysis.genepanel_name, sample.Analysis.genepanel_version).in_(genepanels),
        or_(
