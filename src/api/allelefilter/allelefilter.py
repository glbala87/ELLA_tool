...
class AlleleFilter(object):
    ...
    def filter_alleles(self, filter_config_id, gp_allele_ids, analysis_allele_ids):
        ...
        analysis_allele_result = {
            a_id: {"excluded_allele_ids": dict()} for a_id in analysis_allele_ids
        }
                    ...
                    for gp_key, allele_ids in filtered_gp_allele_ids.items():
                        if gp_key in gp_allele_ids:
                            ...
                            gp_allele_result[gp_key]["excluded_allele_ids"][name] = sorted(
                                list(gp_allele_ids[gp_key] & allele_ids)
                            )
                            ...
                            for (a_id, analysis_gp_key) in analysis_genepanels.items():
                                if analysis_gp_key == gp_key:
                                    ...
                                    analysis_allele_result[a_id]["excluded_allele_ids"][
                                        name
                                    ] = sorted(list(analysis_allele_ids[a_id] & allele_ids))
