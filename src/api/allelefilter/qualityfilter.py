...
class QualityFilter(object):
    ...
    def filter_alleles(self, analysis_allele_ids, filter_config):
        ...
        for analysis_id, allele_ids in analysis_allele_ids.iteritems():
            ...
            quality_filtered = self.session.query(
                genotype.Genotype.allele_id,
                genotype.Genotype.secondallele_id,
            ).join(
                sample.Sample
            ).filter(
                or_(
            ...
            quality_filtered = [a[0] for a in quality_filtered] + [a[1] for a in quality_filtered if a[1]]
