...
@router.get("/search")
def search(q: str, session: Session = Depends(get_session), user=Depends(get_current_user)):
    ...
    if not search_query.check():
        ...
        # Search analysis
        analyses = _search_analysis(session, search_query, genepanels)
        analysis_ids = [a["id"] for a in analyses]
        analysis_interpretations = _get_analysis_interpretations(session, analysis_ids)
        ...
        if search_query.check():
            # Search allele
            alleles = _search_allele(session, search_query, genepanels)
            allele_ids = [a["id"] for a in alleles]
            allele_interpretations = _get_allele_interpretations(session, allele_ids)
            for al in alleles:
...
def _get_analysis_interpretations(session, analysis_ids):
    interpretations = (
...
def _get_allele_interpretations(session, allele_ids):
    interpretations = (
...
def _get_analyses_filters(session, search_query, genepanels):
    filters = list()
...
def _get_allele_results_ids(session, search_query):
    ...
    if search_query.is_valid_freetext():
        if search_query.is_hgvs():
            hgvs_cte = _search_allele_hgvs(session, search_query).cte("hgvsc")
            filters.append(allele.Allele.id.in_(select([hgvs_cte.c.allele_id])))
        elif search_query.is_position():
            position_cte = _search_allele_position(session, search_query)
            if position_cte:
...
def _search_allele_hgvs(session, search_query):
    """
...
def _search_allele_position(session, search_query):
...
def _filter_transcripts_query(session, alleles, genepanels, search_query):
    """
...
def _search_allele(session, search_query, genepanels):

    # CTE for performance
    allele_results_ids = _get_allele_results_ids(session, search_query).limit(ALLELE_LIMIT).cte()
    ...
    _filter_transcripts_query(session, allele_data, genepanels, search_query)
    return allele_data


def _search_analysis(session, query, genepanels):
    analyses = (
        session.query(sample.Analysis)
        .filter(*_get_analyses_filters(session, query, genepanels))
        .distinct()
...
def search_options(q: str, session: Session = Depends(get_session), user=Depends(get_current_user)):
    result = dict()
    query = json.loads(q)
    if query.get("gene"):
        gene_results = (
