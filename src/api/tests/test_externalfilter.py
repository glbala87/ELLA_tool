...
def create_annotation(allele_id, num_stars, num_benign, num_uncertain, num_pathogenic, hgmd_tag):
    ...
    ann['external']['CLINVAR']= {'variant_description': clinical_significance_status[num_stars], 'items': []}

    for n in range(num_benign):
        ann['external']['CLINVAR']['items'].append({
            'rcv': "SCVXXXXXXXX",
            'clinical_significance_descr': 'Benign'
        })
    ...
    if not ann['external']['CLINVAR']['items']:
        ann['external']['CLINVAR']['items'] = [{
            'rcv': 'SCVXXXXXXXX',
            'clinical_significance_descr': "something"
        }]
...
def annotations(draw):
    ...
    for i in range(1,10):
        ...
        annotation_objs.append(create_annotation(i, num_stars, num_benign, num_uncertain, num_pathogenic, hgmd_tag))
...
def hgmd_strategy(draw):
    hgmd_tags = draw(st.lists(st.sampled_from([None, 'FP', 'DM', 'DFP', 'R', 'DP', 'DM?']), unique=True))
    return hgmd_tags
...
def test_externalfilter(session, annotations, clinvar_strategy, clinvar_stars,  hgmd_strategy):
    ...
    for obj in annotation_objs:
        existing = session.query(
            ...
            annotation.Annotation.allele_id == obj.allele_id
        ).one()
