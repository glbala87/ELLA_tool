...
def check_latest_log(client, workflow_type, message=None, review_comment=None, priority=None, warning_cleared=None, editable=None):
...
def test_allele_workflow(client, test_database):
    ...
    data = {"message": message}
    resp = client.patch('/api/v1/workflows/alleles/{}/logs/{}/'.format(ALLELE_ID, log_id), data=data)
...
def test_analyses_workflow(client, test_database):
    ...
    message = 'Message 1, edited'
    data = {"message": message}
    resp = client.patch('/api/v1/workflows/analyses/{}/logs/{}/'.format(ANALYSIS_ID, log_id), data=data)
