...
class TestGenepanel(object):
    ...
    def test_create_new(self, test_database, client, session):
        ...
        original = session.query(gene.Genepanel).filter(
            gene.Genepanel.name == original_name,
            gene.Genepanel.version == original_version
        ).one()

        created = session.query(gene.Genepanel).filter(
