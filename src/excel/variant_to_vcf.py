...
class ExcelExporter(object):
    ...
    def parse_exported(self, variants, output):
        ...
        vcf_writer = VcfWriter(
            output, "Exported_{}".format(datetime.datetime.today().strftime("%Y-%m-%d"))
        )
            ...
            for idx, line in enumerate(variants):
                ...
                logging.info(
                    "Processing {} ({} of {})".format(
                        transcript_hgvsc, idx + 1 - number_of_skips, len(variants) - number_of_skips
                    )
                )
                try:
                    try:
                        ...
                        logging.exception(
                            "Error while parsing fields {}, it will not be included.".format(
                                transcript_hgvsc
                            )
                        )
                        continue
                        ...
                        logging.exception(
                            "Internal error while parsing fields {}, it will not be included.".format(
                                transcript_hgvsc
                            )
                        )
                        continue
                    ...
                    if var_c.posedit.pos.uncertain:
                        logging.warning(
                            "Position uncertain for fields {}, it will not be included!".format(
                                transcript_hgvsc
                            )
                        )
                        continue
                    ...
                    # del
                        ...
                        vcf_data.update({"pos": start - 1, "ref": ref, "alt": alt})
                    # ins
                    elif ref is None:
                        _, _, _, ref, alt = allele_creator.insertion(
                            chrom, start - 1, end, inserted=alt
                        )
                        vcf_data.update({"pos": start, "ref": ref, "alt": alt})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "variantlist", metavar="N", type=str, nargs=1, help="TSV file with variant assessments"
    )
    parser.add_argument(
        "--reference",
        action="store",
        dest="reference",
        required=True,
        help="Path to genome reference file",
    )
    parser.add_argument(
        "--outputdir",
        action="store",
        dest="outputDirectoryPath",
        required=False,
    ...
    output_file = os.path.join(
        output_dir, "{base_filename}.vcf".format(base_filename=os.path.basename(input_filename))
    )
