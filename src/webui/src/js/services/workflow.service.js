...
@Inject('$rootScope', 'Allele', 'WorkflowResource', '$uibModal', '$location')
class WorkflowService {
    constructor(rootScope, Allele, WorkflowResource, ModalService, LocationService) {
    ...
    canFinalize(type, interpretation, alleles, config) {
        ...
        if (
            ...
            has_required_status = config.user.user_config.workflows[
                type
            ...
            all_classified = alleles.every((a) => {
                if (a.id in interpretation.state.allele) {
                    ...
                    let has_classification = Boolean(
                        AlleleStateHelper.getClassification(a, allele_state)
                    )
                    let not_reused_outdated = true
                    if (AlleleStateHelper.isAlleleAssessmentReused(allele_state)) {
                        not_reused_outdated = !AlleleStateHelper.isAlleleAssessmentOutdated(
                            a,
                            config
                        )
                    }
    ...
    confirmCompleteFinalize(
        ...
        let modal = this.modalService.open({
            ...
            controller: [
                ...
                'type',
                '$uibModalInstance',
                ConfirmCompleteInterpretationController
            ],
            size: 'lg',
            resolve: {
                type: () => type,
    ...
    prepareAlleleAssessmentsForApi(
        allele,
        allelestate,
        analysis_id = null,
        genepanel_name = null,
    ...
    prepareReferenceAssessmentsForApi(
        allelestate,
        analysis_id = null,
        genepanel_name = null,
        genepanel_version = null
