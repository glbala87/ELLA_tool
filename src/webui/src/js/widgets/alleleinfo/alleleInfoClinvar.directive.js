...
app.component('alleleInfoClinvar', {
    templateUrl: 'ngtmpl/alleleInfoClinvar-new.ngtmpl.html',
    controller: connect(
        {
...
@Directive({
    selector: 'allele-info-clinvar-old',
    scope: {
        allele: '='
    },
...
export class AlleleInfoClinvar {
        ...
        $scope.$watch(
            ...
            () => {
                if (this.hasContent()) {
                    ...
                    this.revstars = this.config.annotation.clinvar.clinical_significance_status[
                        this.revtext
                    ]
                }
            }
    ...
    formatClinvar() {
        ...
        if (this.hasContent()) {
            for (
                let idx = 0;
                idx < this.allele.annotation.external.CLINVAR['items'].length;
                idx++
            ) {
