...
export class ImportSingleController {
    ...
    getSummary() {
        ...
        if (this.importData.isVariantMode()) {
            ...
            summary['gene panel'] = this.importData.importSelection.genepanel
                ? `${this.importData.importSelection.genepanel.name} ${this.importData.importSelection.genepanel.version}`
                : ''
            ...
            summary['analysis'] = this.importData.importSelection.analysis
                ? this.importData.importSelection.analysis.name
                : ''
        }
    ...
    analysisIsStarted() {
        if (!this.importData.importSelection.analysis) return false
            ...
            return (
                ...
                this._analysisOngoing(statuses) ||
                this._analysisInReview(statuses)
            )
        }
    }
    ...
    checkAnalysisStatus() {
        ...
        let interpretation_date = this.filter('date')(
            last_interpretation.date_last_update,
            'dd-MM-yyyy HH:mm'
        )
    ...
    findExistingAnalysis() {
        ...
        let p_name = this.analysisResource.get(
            { name: { $ilike: `%${this.importData.importSelection.analysisName}%` } },
            2
        )
