...
export class ReferenceEvalModalController {
        ...
        this.sources = {
            ...
            phase: {
                ...
                elements: [
                    {
                        type: 'button',
                        options: [['Cis', 'in_cis_pathogenic'], ['Trans', 'in_trans_pathogenic']],
                        store: 'ref_phase'
                    }
                ],
                optional_dep: 'segregation'
            ...
            prediction: {
                ...
                elements: [
                    {
                        type: 'button',
                        options: [['Pathogenic', 'pat'], ['VUS', 'vus'], ['Neutral', 'neu']],
    ...
    shouldShow(source) {
        ...
        if ('show_when_selection' in this.sources[source]) {
            ...
            return (
                this.referenceAssessment.evaluation[this.sources[source].show_when_selection[0]] ===
                this.sources[source].show_when_selection[1]
            )
        } else {
            return should_show
        }
    ...
    cleanup() {
        for (let [source, source_data] of Object.entries(this.sources)) {
            ...
            if (
                !this.getSources().includes(source) ||
