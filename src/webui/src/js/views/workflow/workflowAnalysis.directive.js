    ...
    constructor(
        ...
        this.scope.$watch(
            ...
            () => {
                ...
                this.setupNavbar()
                if (analysis && analysis.warnings && analysis.warnings.length) {
        ...
        this.workflowResource.getCollisions('analysis', this.analysisId).then(result => {
            if (result.length > 0) {
                ...
                if (result.length > 1) {
                    html += `are currently ${
                        result.length
                    } variants being worked on in other workflows.</h4>`
                } else {
                    html += `is currently ${
                        result.length
                    } variant being worked on in another workflow.</h4>`
                }

                for (let c of result) {
                    html += `<h3> ${c.allele.annotation.filtered[0].symbol} ${c.allele.annotation
                        .filtered[0].HGVSc_short || c.allele.getHGVSgShort()}`
                    html += ` ${
                        c.user ? ' is worked on by ' + c.user.full_name : 'is pending review'
                    } ${
                        c.type === 'analysis' ? 'as part of an analysis' : 'in variant workflow'
                    }</h3>`
                }
