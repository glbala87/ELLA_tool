...
rules = [
    ...
    {
        "code": "REQ_>=4affected",
        "rule": {"refassessment.*.ref_population_affecteds": "in_many_aff"},
    },
    ...
    {
        "code": "REQ_crit_domain_hotspot",
        "rule": {"refassessment.*.ref_domain_overlap": {"$in": ["critical_domain", "mut_hotspot"]}},
    },
    {"code": "REQ_crit_site", "rule": {"refassessment.*.ref_domain_overlap": "critical_site"}},
    {
        "code": "REQ_domain_no_benign",
        "rule": {"refassessment.*.ref_domain_benign": "no_benign_variation"},
    },
    ...
    {
        "code": "REQ_in_trans_pathogenic",
        "rule": {"refassessment.*.ref_phase": "in_trans_pathogenic"},
    },
    {"code": "REQ_in_trans_pathogenic", "rule": {"genomic.phase": "in_trans_pathogenic"}},
    {
        "code": "REQ_inframe",
        "rule": {"transcript.consequences": {"$in": ["inframe_insertion", "inframe_deletion"]}},
    },
    {"code": "REQ_less_common", "rule": {"frequencies.commonness": "less_common"}},
    {
        "code": "REQ_missense",
        "rule": {
            "transcript.consequences": {"$in": ["missense_variant", "protein_altering_variant"]}
        },
    },
    ...
    # Manual edit
    {
        "code": "REQ_no_splice_effect",
        "rule": {
            "$or": [
                {
                    "prediction.splice_Effect": {
                        "$in": ["predicted_conserved", "consensus_not_affected", "not_transcribed"]
                    }
                },
                {"prediction.splice_Effect": {"$not": {"$in": ["de_novo"]}}},
            ]
        },
    },
    {"code": "REQ_no_splice_effect", "rule": {"prediction.splice_Effect_manual": "no_effect"}},
    ...
    {"code": "REQ_non_repeat", "rule": {"prediction.repeat": "non_repeat"}},
    ...
    {"code": "REQ_repeat", "rule": {"prediction.repeat": "repeat"}},
    ...
    # Manual edit
    {
        ...
        "rule": {
            "$$aggregate": {
                "$and": [
                    "REQ_null_variant",
                    {"$in": ["REQ_GP_LOF_missense", "REQ_GP_LOF_only"]},
                    {
                        "$or": [
                            "REQ_not_in_last_exon",
                            {"$all": ["REQ_in_last_exon", "REQ_GP_last_exon_important"]},
                        ]
                    },
                ]
            }
    ...
    {
        "code": "PS1",
        "rule": {"$$aggregate": {"$all": ["REQ_overlap_pat", "REQ_same_aa", "REQ_overlap_HQ"]}},
    },
    ...
    {
        "code": "PSxPM1",
        "rule": {"$$aggregate": {"$all": ["REQ_crit_site", "REQ_domain_no_benign"]}},
    },
    {"code": "PSxPM1", "rule": {"prediction.domain": "critical_site"}},
    {
        "code": "PSxPP1",
        "rule": {"$$aggregate": {"$all": ["REQ_segregation", "REQ_segregation_HQ"]}},
    },
    ### PM*
    {"code": "PM1", "rule": {"prediction.domain": "critical_domain"}},
    {
        "code": "PM1",
        "rule": {"$$aggregate": {"$all": ["REQ_crit_domain_hotspot", "REQ_domain_no_benign"]}},
    },
    # PM2 (no rule)
    ...
    {"code": "PM4", "rule": {"$$aggregate": {"$all": ["REQ_inframe", "REQ_non_repeat"]}}},
    {
        ...
        "rule": {
            "$$aggregate": {
                "$all": ["REQ_overlap_pat", "REQ_novel_aa", "REQ_aa_similar", "REQ_overlap_HQ"]
            }
        },
    },
    ...
    {
        "code": "PMxPP1",
        "rule": {"$$aggregate": {"$all": ["REQ_segregation", "REQ_segregation_MQ"]}},
    },
    {
        "code": "PMxPS1",
        "rule": {"$$aggregate": {"$all": ["REQ_overlap_pat", "REQ_same_aa", "REQ_overlap_MQ"]}},
    },
    {
        "code": "PMxPS3",
        "rule": {"$$aggregate": {"$all": ["REQ_abnormal_protein", "REQ_protein_MQ"]}},
    },
    ...
    # Manual edit
    {
        ...
        "rule": {
            "$$aggregate": {
                "$and": [
                    {"$all": ["REQ_missense", "REQ_GP_missense_uncommon"]},
                    {"$in": ["REQ_GP_LOF_missense", "REQ_GP_missense_only"]},
                ]
            }
        },
    },
    ...
    {
        "code": "PP3",
        "rule": {"prediction.splice_Effect_manual": {"$in": ["predicted_lost", "de_novo"]}},
    },
    {"code": "PP3", "rule": {"prediction.ortholog_conservation": "conserved"}},
    {"code": "PP3", "rule": {"prediction.paralog_conservation": "conserved"}},
    # PP4 (no rule)
    ...
    # Manual edit
    {
        ...
        "rule": {
            "$$aggregate": {
                "$and": ["REQ_no_freq", {"$not": {"$in": ["REQ_less_common", "REQ_hi_freq"]}}]
            }
        },
    },
    # Manual edit
    ...
    {
        "code": "BS4",
        "rule": {"$$aggregate": {"$all": ["REQ_no_segregation", "REQ_segregation_HQ"]}},
    },
    ### BP*
    ...
    {"code": "BP4", "rule": {"prediction.splice_Effect_manual": "no_effect"}},
    {"code": "BP4", "rule": {"prediction.ortholog_conservation": "non-conserved"}},
    {"code": "BP4", "rule": {"prediction.paralog_conservation": "non-conserved"}},
    # BP5 (no rule)
    {"code": "BP6", "rule": {"external.[Trusted source]": "Benign"}},
    {
        "code": "BP7",
        "rule": {"$$aggregate": {"$all": ["REQ_no_aa_change", "REQ_no_splice_effect"]}},
    },
    ...
    {
        "code": "BPxBS4",
        "rule": {"$$aggregate": {"$all": ["REQ_no_segregation", "REQ_segregation_MQ"]}},
    }
    #    {
    #        "code": "XXX",
